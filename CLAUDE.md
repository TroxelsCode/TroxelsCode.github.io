...
- `js/hero.js` - the component's only host. ES module. Mounts **all three tiers**, one per `.hero-layer` inside `#hero-mount`, and drives the scroll sequence. Picks landscape or portrait tier data from a `matchMedia` width query, picks pinned-vs-stacked layout from width **or** `prefers-reduced-motion`, re-lays-out on either crossing, and defers mounting entirely while the narrow-screen `<details>` disclosure is collapsed. See "Homepage build" and "Phase 2b" below.
...
- **The summary is `display: none` above the breakpoint**, so the disclosure reads as a plain
  wrapper on a desktop. That is exactly why `watchLayout()` force-opens it when crossing
