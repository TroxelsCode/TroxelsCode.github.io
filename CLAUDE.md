...
- **Both are suppressed under `prefers-reduced-motion` and the packets toggle does NOT cover
  ...
  are invisible from a reduced-motion session and need a motion-allowed one to confirm.
...
- **Standby site links now dash-march (2026-08-07), large tier only.** User request: the site
  ...
  states; **the visible motion is not user-confirmed yet** and cannot be checked from the RDP
  session, since neither march has a packets-toggle style override.
