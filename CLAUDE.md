...
- **The Pages API is NOT the authoritative build status - `gh run list` is.** The legacy branch-based deploy still runs as an Actions workflow called `pages-build-deployment`, and `gh api .../pages/builds/latest` lags or contradicts it. Observed twice on 2026-08-06, once in each direction: during the incident the API reported `status: "building"` for a run that had already **completed as `failure`**, and later on a fully healthy day it reported the previous commit as latest when the newest commit had **already built successfully**. Both times the workflow list was correct. So check with:

  ```sh
  gh run list --workflow="pages-build-deployment" --limit 5
  ```

  A `success` row against your HEAD sha means you are deployed, whatever the Pages API says. Duration is the tell there too: normal rows are 36-45s, and the incident rows read `30m9s` and `1h3m22s`.
...
- **Liquid cannot currently break a build here, and it is worth knowing exactly why** (established 2026-08-06 after first documenting the opposite - the correction matters more than the original claim). **No file in this repo carries YAML front matter**: every served file starts with real content (`# CLAUDE.md`, `<!DOCTYPE html>`, ...). Jekyll only runs Liquid and the Markdown converter on files that *have* front matter; everything else is a `StaticFile`, copied verbatim. So nothing here is Liquid-parsed, and a literal doubled brace in Markdown is harmless today. The Jekyll build does exactly one thing for this repo: `EntryFilter`'s underscore-directory exclusion that keeps `_tests/` and `_icons/` off the live domain.
  - **The conditional to watch**: this stops being true the moment any file gains front matter, which is exactly what adopting layouts or includes would require (contemplated at ~5 pages - see the no-build-step decision below). At that point Liquid runs over the raw file *before* Markdown rendering, a backtick code span does **not** protect a delimiter, and documenting the hazard literally would introduce it. If that day comes, write such patterns as character classes - `grep -cE '[{][{]' *.md` - rather than spelling them out.
  - So when a build fails, do **not** reach for a Liquid explanation while the repo is front-matter-free. Check `duration` and the workflow list first (above); `duration: 0` means the job never ran and the cause is GitHub-side.
- **Do not confirm a deploy by polling the build's `commit` field - it misattributes.** When two pushes land close together, the build that actually compiles your tree can be stamped with the PREVIOUS commit SHA, so a poll loop waiting for your SHA to appear times out while the deploy has in fact succeeded (hit 2026-08-06: `469c95d` never appeared, yet its content was live). **Verify by content instead**: fetch a page from `https://troxeltech.com/` and grep for a string your commit introduced. For doc-only commits, `CLAUDE.md` is served as `/CLAUDE.html`, which makes a convenient probe.
- **Deploy**: push to `main`; GitHub Pages auto-builds from the branch root (legacy Pages build, no Actions workflow). Everything on `main` is publicly served **except underscore-prefixed directories**, which Jekyll's `EntryFilter` skips unless they are one of its known dirs (`_posts`, `_layouts`, ...) or are listed in an `include:` key. That is the only thing keeping `_tests/` off the live domain - it is not merely unlinked, it is absent from the built site. **Root `.md` files ARE published as HTML.** Confirmed 2026-08-06: `https://troxeltech.com/CLAUDE.html` returns 200 and serves this entire file, and `network-topology-prototype-spec.md` is exposed the same way. (`README.md` is the exception - Jekyll does not emit `/README.html`.) Underscore *directories* are excluded; root markdown is not. The repo is public so nothing here is secret, but serving internal working notes from the professional domain is a different question from having them on github.com - **flagged to the user 2026-08-06, awaiting a decision**. The fix, if wanted, is a `_config.yml` with `exclude: [CLAUDE.md, network-topology-prototype-spec.md]`; do NOT move or rename `CLAUDE.md`, which has to stay at the repo root for Claude Code to find it. **Never add a `.nojekyll` file**: it bypasses the Jekyll build entirely and would start publishing `_tests/`. Same caveat if this ever migrates to an Actions-based Pages workflow - `actions/upload-pages-artifact` uploads the whole tree unless a Jekyll build runs first. (The files stay browsable on github.com either way; the repo is public. The goal is "not on the live domain", not "secret".)
