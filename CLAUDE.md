...
- **Ten-second way to rule out the content cause** before blaming an outage: a docs-only commit still goes through Jekyll, so Liquid syntax in root Markdown genuinely can break a build. Run `grep -cE '[{][{]' *.md` and `grep -cE '[{][%]' *.md` - all zero means there is no Liquid to trip over and the failure is not yours. Worth running because it converts "probably GitHub" into "definitely not the repo". **Write those patterns as character classes, exactly as above, and never spell the raw sequences out in this file.** Jekyll runs Liquid over the raw Markdown *before* Markdown rendering, so a backtick code span does not protect them - documenting the hazard literally would introduce it. This was caught here on 2026-08-06 immediately after being written the naive way.
