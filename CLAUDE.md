...
- **EXHIBIT #2 (botnet swarm) IS IN PROGRESS ON THE BRANCH `swarm-exhibit`, NOT ON `main`.** Phases 1-4 are built and committed there; nothing is pushed and nothing is live. **The full design lives in `botnet-swarm-spec.md` at the repo root** - read that before touching anything under `swarm/` or `js/swarm.js`, because this section deliberately does not duplicate it. That file is temporary scaffolding: per its own Phase 5 it gets migrated into this file and then DELETED, along with its `_config.yml` exclude. Until that happens this bullet is the pointer.
  - A boids simulation where a botnet swarms three servers, across three tiers that vary **only** by defense sophistication (none / rate limiting / rate limiting + tarpit). Canvas 2D rather than SVG, because a few hundred agents across three tiers is too many DOM nodes to mutate per frame.
  - **The engine is deterministic under an injected seeded RNG and a fixed timestep**, which is the single most load-bearing decision in the build: it is what lets `_tests/swarm-tests.html` assert whole runs rather than single transitions, including the exhibit's actual thesis (outages and mean node load both fall as defense improves). 27 tests. Never pass a raw frame delta into `step()`.
  - **Two things the user benched on 2026-08-08 and does not want quietly resolved**: visitor interactivity (so the exhibit is observational and correctly ships **no** `.exhibit-directions` paragraph), and a honeypot/decoy node for tier 3. Build as if the answer is no.
  - **All copy is placeholder** and marked as such at every definition site, pending the user seeing it run. So are the tuning constants - see the tunables table in the spec.
  - **BASELINE ACCEPTED by the user 2026-08-09** after watching it run. Further tuning is to happen in separate focused sessions, not by extending the build session. Stopped deliberately before Phase 5 (deploy, live verification, doc migration).
  - **A reusable lesson from a bug the live page found and the test suite could not:** the renderer originally gated simulation on each tier's own `IntersectionObserver`, so each tier only advanced while it personally sat in the viewport. Since the scoreboard totals are cumulative and are meant to be compared BETWEEN tiers, parking the page mid-exhibit produced a confident but completely false reading. **Cumulative counters plus per-instance visibility gating give numbers that cannot be compared** - gate every instance together, or display a rate instead of a total. Applies to any future exhibit, not just this one.
