...
This file provides guidance to every AI coding agent working in this repository. It is the
single shared instruction file: Codex reads it directly, and Claude Code reads it through the
`@AGENTS.md` import in `CLAUDE.md`. **Write shared knowledge here, not in `CLAUDE.md`**, which
holds only notes about Claude's own tools. Two copies of the same rule is the drift failure
described under the documentation map below.
...
**If a future session needs private context to do good work, it does not go here.** Keep it in a file *outside* the repository and load it from there - an `@path` import line in `CLAUDE.md` for Claude Code, or the global `~/.codex/AGENTS.md` for Codex, which has no import mechanism - so no `git add` can publish it by accident. A gitignored file inside the repo is weaker protection - a broad `git add` can still be talked into staging it, and nothing about the filename warns you at review time.
...
- **These are plain paths, deliberately NOT `@path` imports.** Claude Code expands an
  `@path` import into context at session start, recursively (and since `CLAUDE.md`
  imports this file, an import written here is expanded too), so writing
...
- **`_docs/` is underscore-prefixed on purpose.** Jekyll's `EntryFilter` drops
  ...
  root.** `AGENTS.md` and `CLAUDE.md` are the exceptions only because Codex and Claude
  Code require them there, which is why each needs an explicit exclude. The repo is public either way;
...
- **Python 3.14.6 is installed** and bare `python` resolves in all shells (confirmed 2026-07-13 in PowerShell and Git Bash after a full VS Code restart). Historical gotcha worth remembering: an agent's shells inherit the environment of the process that launched the agent (here, the VS Code host), so PATH changes made while that host is running (e.g. installing Python) are invisible to the tools until the host is fully restarted; restarting only the agent session is not enough.
...
| A quirk of one agent's own tooling (a specific app pane, tool name, or memory system) | that agent's file, e.g. `CLAUDE.md` |
