...
shell invariants every exhibit must honor all live in `AGENTS.md`. Read that first;
...
- **Failover is instant, with no simulated timeout.** A click recomputes and repaints
  ...
  deferred. See "Settled - do not reopen" in `AGENTS.md` for the closure note; do not resurface it as a candidate.
...
rather than forcing a dark theme (see "Theme decision" in `AGENTS.md`). That made
...
sticky header - stays in `AGENTS.md` and is not repeated here.
...
- **The fallback copy states the OUTCOME per tier, and every clause of it is a claim about
  ...
  `AGENTS.md`). It used to describe the picture - "an internet uplink into a firewall, a
...
- **The fallback also carries ONE static SVG frame, and the scenario it freezes was chosen,
  not arbitrary** (added 2026-08-10; the shared rules are under invariant #2 in `AGENTS.md`).
...
- **Keyboard support for nodes: CLOSED FOR CONSIDERATION 2026-08-08, will not be built.**
  ...
  `AGENTS.md`). Reasoning: this site's audience is friends, family, and hiring
...
- **Verified as far as this environment allows**: correct tier at every width on fresh load,
  ...
  the resize note in Environment in `AGENTS.md`.
...
- **It mounts on the summary's CLICK, not on `toggle`, and the ordering is the point**
  (changed 2026-08-13; the shared rule is invariant #1 in `AGENTS.md`). `<details>` fires
...
- Verified 2026-08-07 headlessly in both motion states, using the `matchMedia` shim from the
  Commands section of `AGENTS.md` for the motion-allowed half. Under the headless reduced-motion default the toggle starts
...
do-not-reopen note, not because anything is pending. `AGENTS.md` carries a one-line
